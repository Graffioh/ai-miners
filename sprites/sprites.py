...
class SpriteDirectionCNN(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, input_channels=4):
...
def test_sprites(model, test_loader, criterion):
    ...
    for i in range(NUM_CLASSES):
        if class_total[i] > 0:
            ...
            print(f"  Direction {i} ({direction_names[i]}): {acc:.1f}% ({class_correct[i]}/{class_total[i]})")
        else:
            print(f"  Direction {i} ({direction_names[i]}): No samples")

    return test_accuracy, test_loss

def save_model(model, train_losses, test_accuracy, test_loss, save_path="/content/drive/MyDrive/neurodragon_berto_luca/sprite_direction_model.pth"):
...
def main():
    train_dataset_path = "/content/drive/MyDrive/berto_dataset_sprites_shadowless/train"
    test_dataset_path = "/content/drive/MyDrive/berto_dataset_sprites_shadowless/test"

    transform = transforms.Compose([
        transforms.ToPILImage(),
        transforms.ToTensor(),
        transforms.Normalize(
            mean=[0.013441166840493679, 0.010885078459978104, 0.010833792388439178, 0.04079267755150795],
            std=[0.07362207025289536, 0.06255189329385757, 0.0627019852399826, 0.18982279300689697]
        )
    ])
    ...
    test_accuracy, test_loss = test_sprites(model, test_data, criterion)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path=f"/content/drive/MyDrive/neurodragon_berto_luca/sprite_direction_model_{timestamp}.pth"
    save_model(model, train_losses, test_accuracy, test_loss, save_path)
